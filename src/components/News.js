...
const News=(props)=> {
  ...
  //   const url= `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=627b82ff378a4a79b62696b9830a8096&page=${page}&pageSize=${props.pageSize}`;
  ...
  useEffect(() => {
    document.title = `${capitalizeFirstLetter(props.category)} - NewUse`;
    updateNews();
  ...
  const fetchMoreData = async()=>{
    const url= `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=627b82ff378a4a79b62696b9830a8096&page=${page+1}&pageSize=${props.pageSize}`;
    setpage(page+1)
  ...
  <>
      <h2
        className={`text-capitalize text-${props.mode==='dark'?'danger':'dark'}`} 
        style={{
          display: 'flex',
      ...
      <div className='row'>
         ...
         {articles.map((element)=>{
          ...
          <Newsitems
            ...
            imgUrl={element.urlToImage?element.urlToImage:"https://techcrunch.com/wp-content/uploads/2021/09/GettyImages-1171712648.jpg?w=600"}
            newsUrl={element.url?element.url:"https://www.wsj.com/articles/spacex-returns-private-astronauts-to-earth-after-three-days-in-orbit-11632006524"}
            time={element.publishedAt?element.publishedAt:"recent"}
            mode={props.mode}
            author={element.author?element.author:"unknown"}
            source={element.source.name?element.source.name:"unavailable"}
            />
        </div>
      })}
      </div>
      ...
      {/* <div className="container d-flex justify-content-between">
        <button disabled={state.page<=1} type="button" className={`btn btn-outline-${props.mode==='dark'?'secondary':'primary'} text-${props.mode==='dark'?'danger':''}`} onClick={handlePrev}> &larr; Prev-Page</button>
        <button disabled={state.page+1>Math.ceil(state.totalResults/props.pageSize)} type="button" className={`btn btn-outline-${props.mode==='dark'?'secondary':'info'} text-${props.mode==='dark'?'danger':''}`} onClick={handleNext}>Next-Page &rarr;</button>
      </div> */}
